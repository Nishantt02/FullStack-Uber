    ...
    socket.on('join', async (data) => {
      try {
        ...
        if (userType === 'user') {
          ...
          if (updatedUser) console.log(`✅ User (${userId}) socket ID updated: ${socket.id}`);
          else console.log(`❌ No user found with ID: ${userId}`);
        } else if (userType === 'captain') {
          const updatedCaptain = await captainModel.findByIdAndUpdate(
            userId,
            { socketId: socket.id },
          ...
          if (updatedCaptain) console.log(`✅ Captain (${userId}) socket ID updated: ${socket.id}`);
          else console.log(`❌ No captain found with ID: ${userId}`);
        } else {
          console.log(`❌ Unknown userType: ${userType}`);
        }
      } catch (err) {
